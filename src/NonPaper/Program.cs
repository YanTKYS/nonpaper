...
builder.Services.Configure<FormOptions>(o =>
    o.MultipartBodyLengthLimit = builder.Configuration.GetValue<long>("Upload:MaxFileSizeBytes", 104857600)
        * builder.Configuration.GetValue("Upload:MaxDocuments", 20));
...
app.MapPost("/api/events", async (CreateEventRequest request, EventService service, CancellationToken ct) =>
{
    try
    {
        var x = await service.CreateAsync(request, ct);
        return Results.Created($"/api/events/{x.Event.Id}", new { @event = x.Event, managementToken = x.Token });
    }
    catch (ArgumentException ex) { return Problem(ex.Message); }
});

app.MapGet("/api/events/{id}/manage", async (string id, HttpRequest req, EventService service, CancellationToken ct) =>
{
    var a = await Auth(id, req, service, ct);
    return a.error ?? Results.Ok(a.value);
});

app.MapGet("/api/events/{id}", async (string id, IEventRepository repo, CancellationToken ct) =>
{
    try
    {
...
app.MapPost("/api/events/{id}/documents", async (string id, HttpRequest req, EventService service, IEventRepository repo, ILogger<Program> log, CancellationToken ct) =>
{
    if (!MutatingAllowed(req)) return Problem("不正な要求です。", 403);
    var a = await Auth(id, req, service, ct);
    if (a.error is not null) return a.error;
    if (a.value!.Status != "draft") return Problem("資料を変更できるのは下書き状態だけです。", 409);

    var files = req.Form.Files;
    if (files.Count == 0) return Problem("PDFを選択してください。");
    if (a.value.Documents.Count + files.Count > service.MaxDocuments) return Problem("資料数が上限を超えています。", 413);

    foreach (var file in files)
    {
        if (file.Length == 0 || file.Length > service.MaxFileSize) return Problem("ファイルサイズが上限を超えています。", 413);
        if (!string.Equals(Path.GetExtension(file.FileName), ".pdf", StringComparison.OrdinalIgnoreCase))
            return Problem("PDF以外のファイルは登録できません。");

        await using var input = file.OpenReadStream();
        var header = new byte[5];
        if (await input.ReadAsync(header, ct) != 5 || !header.SequenceEqual("%PDF-"u8.ToArray()))
            return Problem("PDF以外のファイルは登録できません。");

        var docId = EventService.NewId();
        ...
        a.value.Documents.Add(new DocumentRecord
        {
            Id = docId,
            Title = Path.GetFileNameWithoutExtension(safeName),
            OriginalFileName = safeName,
            StoredFileName = docId + ".pdf",
            Order = a.value.Documents.Count + 1,
            FileSize = file.Length,
            UploadedAt = DateTimeOffset.Now,
        });
        log.LogInformation("イベント {EventId} に資料 {DocumentId} を登録しました", id, docId);
    }

    a.value.UpdatedAt = DateTimeOffset.Now;
    await repo.SaveAsync(a.value, ct);
    return Results.Ok(a.value.Documents.OrderBy(x => x.Order));
}).DisableAntiforgery();

app.MapPut("/api/events/{id}/documents/{docId}", async (string id, string docId, UpdateDocumentRequest body, HttpRequest req, EventService service, IEventRepository repo, CancellationToken ct) =>
{
    if (!MutatingAllowed(req)) return Problem("不正な要求です。", 403);
    var a = await Auth(id, req, service, ct);
    if (a.error is not null) return a.error;
    ...
    var d = a.value.Documents.SingleOrDefault(x => x.Id == docId);
    if (d is null) return Problem("資料が見つかりません。", 404);
...
app.MapPut("/api/events/{id}/documents/order", async (string id, ReorderRequest body, HttpRequest req, EventService service, IEventRepository repo, CancellationToken ct) =>
{
    if (!MutatingAllowed(req)) return Problem("不正な要求です。", 403);
    var a = await Auth(id, req, service, ct);
    if (a.error is not null) return a.error;
    ...
    if (body.DocumentIds.Count != a.value.Documents.Count
        || body.DocumentIds.Distinct().Count() != body.DocumentIds.Count
        || body.DocumentIds.Any(x => a.value.Documents.All(d => d.Id != x)))
        return Problem("資料の並び順が不正です。");

    for (var i = 0; i < body.DocumentIds.Count; i++)
        a.value.Documents.Single(d => d.Id == body.DocumentIds[i]).Order = i + 1;

    await repo.SaveAsync(a.value, ct);
    return Results.Ok(a.value.Documents.OrderBy(x => x.Order));
});

app.MapDelete("/api/events/{id}/documents/{docId}", async (string id, string docId, HttpRequest req, EventService service, IEventRepository repo, ILogger<Program> log, CancellationToken ct) =>
{
    if (!MutatingAllowed(req)) return Problem("不正な要求です。", 403);
    var a = await Auth(id, req, service, ct);
    if (a.error is not null) return a.error;
    ...
    File.Delete(repo.DocumentPath(id, docId));
    a.value.Documents.Remove(d);
    var n = 1;
    foreach (var x in a.value.Documents.OrderBy(x => x.Order)) x.Order = n++;
    await repo.SaveAsync(a.value, ct);
    log.LogInformation("イベント {EventId} の資料 {DocumentId} を削除しました", id, docId);
    return Results.NoContent();
});

app.MapGet("/api/events/{id}/documents/{docId}/content", async (string id, string docId, HttpResponse response, IEventRepository repo, CancellationToken ct) =>
{
    try
    {
        var e = await repo.GetAsync(id, ct);
    ...
    catch (ArgumentException) { return Problem("資料が見つかりません。", 404); }
});
