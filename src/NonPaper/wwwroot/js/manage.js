...
async function load() {
  try { current = await api(`/api/events/${eventId}/manage`, {}, true); title.textContent=current.title; description.textContent=current.description||'（説明なし）'; dates.textContent=`${localDate(current.startsAt)} ～ ${localDate(current.endsAt)}`; status.textContent=labels[current.status]; count.textContent=`${current.documents.length}件`; deleteTitle.textContent=current.title; upload.href=`/upload.html?event=${encodeURIComponent(eventId)}&token=${encodeURIComponent(token)}`; updateActions(); details.classList.remove('hidden'); danger.classList.remove('hidden'); } catch(e){showError(e);}
}
document.querySelectorAll('[data-status]').forEach(b=>b.addEventListener('click',async()=>{const next=b.dataset.status;if(next==='closed'&&!confirm('会議を終了すると、参加者は資料を閲覧できなくなり、再公開できません。\n会議を終了しますか？'))return;if(next==='draft'&&!confirm('下書きに戻すと、参加者は資料を閲覧できなくなります。\n下書きに戻しますか？'))return;try{await api(`/api/events/${eventId}/status/${next}`,{method:'POST'},true);await load();}catch(e){showError(e);}}));
